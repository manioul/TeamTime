...
{foreach from=$lines item=value}
	{if $lineNb < 2}<th{else}<td{/if}{if isset($value.id)} id="{$value.id}"{/if}{if isset($value.classe)} class="{$value.classe}"{/if}{if isset($value.title)} title="{$value.title|escape:html}"{/if}{if isset($value.colspan)} colspan="{$value.colspan}"{/if}>{if isset($value.navigateur)}{* Construction d'un navigateur entre les cycles pour la case contenant l'année *}
	...
	<div class="shift">{$value.vacation}</div>{elseif isset($value.nom)}{$value.nom}{/if}{if $lineNb < 2}</th>{else}</td>{/if}{/foreach}
</tr>
