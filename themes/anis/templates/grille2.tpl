...
{foreach $lines as $value}
	{if $lines@iteration < 3}<th{else}<td{/if}{if isset($value.id)} id="{$value.id}"{/if}{if isset($value.classe)} class="{$value.classe}"{/if}{if isset($value.title)} title="{$value.title|escape:html}"{/if}{if isset($value.colspan)} colspan="{$value.colspan}"{/if}>{if isset($value.navigateur)}{* Construction d'un navigateur entre les cycles pour la case contenant l'année *}
