...
// updateContact.php
//
// Traitement de la mise à jour des informations de contact des utilisateurs
...
if (empty($_SESSION['ADMIN']) && $_POST['uid'] != $_SESSION['utilisateur']->uid()) {
	print "Vous ne pouvez modifier que vos données...";
	exit;
}
...
if (!empty($_POST['newnb'])) {
	$newPhone = array(	'phone'		=> $_POST['newnb']
...
// Si il y a une nouvelle affectation
if (!empty($_POST['newcentre']) && !empty($_POST['newteam'])
	&& !empty($_POST['newgrade']) && !empty($_POST['newbeginning'])
       	&& !empty($_POST['newend'])) {
	$utilisateur->addAffectation(array(
		'centre'	=> $_POST['newcentre']
		, 'team'	=> $_POST['newteam']
		, 'grade'	=> $_POST['newgrade']
		, 'beginning'	=> $_POST['newbeginning']
		, 'end'		=> $_POST['newend']
		)
	);
}
