...
BEGIN
	...
	-- On désactive la vérification des clés étrangères.
	...
	ALTER TABLE `TBL_EVENEMENTS_SPECIAUX` CHANGE `did` `did` SMALLINT NOT NULL, ADD KEY `did` (`did`), ADD CONSTRAINT `TBL_EVENEMENTS_SPECIAUX_ibfk_2` FOREIGN KEY (`did`) REFERENCES `TBL_DISPO` (`did`) ON DELETE CASCADE ON UPDATE CASCADE;
	ALTER TABLE `TBL_HEURES` CHANGE `did` `did` SMALLINT NOT NULL, ADD KEY `did` (`did`), ADD CONSTRAINT `TBL_HEURES_ibfk_2` FOREIGN KEY (`did`) REFERENCES `TBL_DISPO` (`did`) ON DELETE CASCADE ON UPDATE CASCADE;
	ALTER TABLE `TBL_VACANCES_A_ANNULER` CHANGE `did` `did` SMALLINT NOT NULL, ADD KEY `did` (`did`), ADD CONSTRAINT `TBL_VACANCES_A_ANNULER_ibfk_2` FOREIGN KEY (`did`) REFERENCES `TBL_DISPO` (`did`) ON DELETE CASCADE ON UPDATE CASCADE;

	-- Les vacances ne doivent pas être liées car il faut éditer un titre d'annulation pour des congés déposés avant la suppression
	-- ALTER TABLE `TBL_VACANCES` ADD KEY `sdid` (`sdid`), ADD CONSTRAINT `TBL_VACANCES_ibfk_1` FOREIGN KEY (`sdid`) REFERENCES `TBL_L_SHIFT_DISPO` (`sdid`) ON DELETE CASCADE ON UPDATE CASCADE;
	...
	-- Création d'un index sur briefing
	ALTER TABLE `TBL_GRILLE` ADD INDEX ( `briefing`  ) ;
	ALTER TABLE `TBL_BRIEFING` ADD INDEX ( `description`  ) ;
	ALTER TABLE `TBL_BRIEFING` ADD CONSTRAINT `TBL_BRIEFING_ibfk_1` FOREIGN KEY (`description`) REFERENCES `TBL_GRILLE` (`briefing`) ON DELETE CASCADE ON UPDATE CASCADE;
